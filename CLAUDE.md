...
**Model selection:** each client returns its **best-on-val epoch**, not its final epoch (`client_val` is threaded into `FederatedClient.train_local` as `val_loader`/`val_labels`). See the symmetry rule under gap #3 — this is required for the comparison against federated to mean anything.
...
**⚠️ Model-selection symmetry rule (all three conditions).** Best-on-val selection is NOT federated-only. `train_centralized` takes `val_loader`/`val_labels` (the same pooled `fl_val_loader` the shared-head server selects on) and returns its best epoch; local-only selects per client on that client's own val split (gap #1). Every condition trains the full budget and then returns its best-on-val checkpoint — **no early stopping anywhere**, since stopping one arm early would just swap one asymmetry for another.

Why this is load-bearing: if only the federated arm gets to pick its best snapshot while centralized and local-only are judged at an arbitrary final epoch, federated wins partly from the max-over-N-noisy-evaluations bonus and partly from overfitting protection the others never got. Centralized is the most exposed (it sees the most data per epoch, so it peaks earliest and then degrades unchecked — 60 epochs at RUN_PLAN's settings). That bias points in the direction that flatters the thesis, which is exactly why an examiner will look for it. Any new training condition added later MUST select on val the same way. The per-client val map (`client_val` in `main()`) is therefore built **unconditionally**, not only in personalized mode; building loaders draws no RNG, so shared-head runs stay bit-reproducible.

Measured on a 4-round bert-tiny plumbing run: **8 of 18 local-only clients selected a non-final epoch**, i.e. the old final-epoch rule was reporting a strictly worse-on-val model for nearly half the pool. Do not read accuracy off that run — the point is only that the selection is not a no-op.

**⚠️ RNG trap — adding any eval pass to a training loop (`fl/metrics.py::preserved_rng`).** Iterating **any** `DataLoader` draws an int64 `_base_seed` from the global **CPU** torch RNG — even with `shuffle=False, num_workers=0`. So dropping a val pass into an existing epoch loop silently reshuffles that loop's `train_loader` from the next epoch onward and changes the training trajectory. This was caught empirically here: adding the centralized val pass moved its epoch-2 average loss from 0.620124 to 0.620603 while epoch 1 matched exactly.

Both new val passes are therefore wrapped in `preserved_rng()`, which snapshots and restores torch (CPU + CUDA) and numpy global state so evaluation stays purely observational. Two consequences worth knowing:
- **Do NOT retrofit `preserved_rng` onto the warm-start or federated-server val passes.** Those existed when their RNG streams were established, so their consumption is already baked into every result on disk; wrapping them now would change federated numbers for no benefit.
- The federated phase happens to be insensitive to CPU-RNG drift **on CUDA** (client data order comes from per-client numpy Generators, client selection from the server's own Generator, and GPU dropout draws from the CUDA generator). On a **CPU-only** run dropout draws from the CPU generator, so unguarded eval passes *would* have shifted federated results. Never rely on that accident — guard the eval pass instead.

**Residual asymmetry — known, defensible, and must be stated in the thesis.** Selection *granularity* still differs by condition, because the methods genuinely differ:
- local-only selects an epoch **per client** (18 independent selections — each project would deploy its own best checkpoint chosen on its own val data),
- centralized selects **one** epoch on the pooled val split (it is one model),
- federated shared-head selects **one** global round on the pooled val split,
- federated personalized-head selects one global round by weighted-mean client val, then snapshots each client's head at that round.

So local-only has more degrees of freedom in selection than federated does. This is not an implementation accident and should NOT be "fixed" by crippling local-only: it reflects that federation constrains all clients to a synchronized round while independent training does not. Report it as a deliberate choice (each condition is selected the way it would actually be deployed) and note that it makes local-only a *conservative* comparator — i.e. it biases against the thesis claim, not for it, which is the safe direction to err.
...
- **Do not compare conditions whose model selection differs.** Every condition (local-only, centralized, federated) trains its full budget and returns its **best-on-val** checkpoint. Adding a condition that reports its final epoch instead silently hands the best-on-val arms a free advantage — see the model-selection symmetry rule under gap #3.
