# Experiment Run Plan — FedSP-PEFT / FedSP-PEFT-P (Linux)

Bash conversion of [RUN_PLAN.md](RUN_PLAN.md) for this machine. **RUN_PLAN.md
remains the source of truth for *what* to run** — this file only translates its
commands to bash (backslash line-continuation) and fills in this machine's
paths. If the two ever disagree about a run, fix this file, not the other way
round. [RQs.md](RQs.md) is the source of truth for the RQ wording.

First-time / per-session setup:

```bash
mkdir -p "/mnt/Data/Oni"
cd "/mnt/code/Oni/Shohel Ahmed CSE/federated-story-point-estimation"
source .venv/bin/activate
pip install coral-pytorch>=1.4.0   # one-time: .venv is missing this as of 2026-08-19
```

Paths are split deliberately: the **read-only data corpus** lives under the
project directory
(`/mnt/code/Oni/Shohel Ahmed CSE/federated-story-point-estimation/data_to_train_on_clean`),
while **all outputs** (`--results-root`, `--save-dir`, `--artifact-dir`,
`--out-dir`) are written under `/mnt/Data/Oni/`. Every value is an absolute
path, so each code block below is safe to copy-paste on its own in any shell
session.

**Two GPUs are present on this machine** (`nvidia-smi`: 2× RTX 3090, 24 GB
each). Independent roots (e.g. the RQ1 shared root and the RQ2 personalized
root, or the two μ-sweep roots) can run in parallel on separate GPUs by
prefixing a command with `CUDA_VISIBLE_DEVICES=0` / `CUDA_VISIBLE_DEVICES=1`.
Do not parallelize two commands that share a `--results-root`/`--save-dir` —
`run_experiments.py`'s coarse resume logic checks for finished result files on
disk and two writers to the same seed/condition directory will race.
...
All roots above are created under `/mnt/Data/Oni/`. RQ2 and RQ3 are both
**cross-root** comparisons against the same shared root, so one analysis helper
(`compare_conditions.py`; see Analysis) serves both.
...
python train_federated_dl.py \
    ...
    --save-dir "/mnt/Data/Oni/artifacts_smoke" --seed 42
```
...
```bash
python compute_statistics.py --experiments-root "/mnt/Data/Oni/experiments_temporal_shared" --metric mae --out-dir "/mnt/Data/Oni/experiments_temporal_shared/statistics_mae"
python compute_statistics.py --experiments-root "/mnt/Data/Oni/experiments_temporal_shared" --metric cohen_kappa --out-dir "/mnt/Data/Oni/experiments_temporal_shared/statistics_cohen_kappa"
python compute_statistics.py --experiments-root "/mnt/Data/Oni/experiments_temporal_shared" --metric macro_f1 --out-dir "/mnt/Data/Oni/experiments_temporal_shared/statistics_macro_f1"
```
...
python compare_conditions.py \
    ...
    --metric mae --out-dir "/mnt/Data/Oni/comparisons/rq2_shared_vs_phead_temporal/mae"
python compare_conditions.py \
    --root-a "/mnt/Data/Oni/experiments_temporal_shared" --label-a "Shared-head" \
    --root-b "/mnt/Data/Oni/experiments_temporal_personalized" --label-b "P-head" \
    --metric cohen_kappa --out-dir "/mnt/Data/Oni/comparisons/rq2_shared_vs_phead_temporal/cohen_kappa"
python compare_conditions.py \
    --root-a "/mnt/Data/Oni/experiments_temporal_shared" --label-a "Shared-head" \
    --root-b "/mnt/Data/Oni/experiments_temporal_personalized" --label-b "P-head" \
    --metric macro_f1 --out-dir "/mnt/Data/Oni/comparisons/rq2_shared_vs_phead_temporal/macro_f1"
```

Each `--out-dir` writes `summary.json` (the printed means, Wilcoxon
stat/p-value, Vargha-Delaney Â, Cliff's delta, win counts) and
`paired_values.csv` (the per-(seed,project) pairs — means alone can hide a
project where personalization helps a lot, or not at all, which is itself
worth reporting given the project-specific-semantics framing). Use a
distinct `--out-dir` per metric, same reasoning as `compute_statistics.py`:
the two output filenames are fixed, so reusing one directory across metrics
would overwrite the previous metric's results.
...
python compare_conditions.py \
    ...
    --metric mae --out-dir "/mnt/Data/Oni/comparisons/rq3_lora_vs_fullft_temporal/mae"
python compare_conditions.py \
    --root-a "/mnt/Data/Oni/experiments_temporal_shared" --label-a "LoRA" \
    --root-b "/mnt/Data/Oni/experiments_temporal_fullft" --label-b "Full-FT" \
    --metric cohen_kappa --out-dir "/mnt/Data/Oni/comparisons/rq3_lora_vs_fullft_temporal/cohen_kappa"
python compare_conditions.py \
    --root-a "/mnt/Data/Oni/experiments_temporal_shared" --label-a "LoRA" \
    --root-b "/mnt/Data/Oni/experiments_temporal_fullft" --label-b "Full-FT" \
    --metric macro_f1 --out-dir "/mnt/Data/Oni/comparisons/rq3_lora_vs_fullft_temporal/macro_f1"
```

Each `--out-dir` writes `summary.json` and `paired_values.csv`, same as RQ2 —
a distinct directory per metric since the filenames are fixed and would
otherwise overwrite across metrics.
...
**This machine: 2× RTX 3090, 24 GB each** (RUN_PLAN.md's VRAM table targets a
3090 Ti, same 24 GB class — the arithmetic below applies unchanged). Everything
runs in fp32 (there is no AMP/autocast, no TF32 setting and no gradient
checkpointing in the codebase), so the arithmetic is predictable. At batch
64 / seq 256, RoBERTa-base stores roughly 1.3 GB of activations per layer —
attention scores 201 MB, softmax output 201 MB, FFN intermediate 201 MB, GELU
output 201 MB, plus projections and LayerNorms — so ~16 GB across 12 layers.
That term is the same for both arms.
...
So full fine-tuning should fit with ~5 GB spare on a single GPU. On Linux (no
WDDM spill behavior), an over-budget allocation raises a normal CUDA OOM instead
of silently degrading — watch `nvidia-smi` during the pre-check; a process
sitting near 24 GB with no OOM traceback means it fit.
...
python train_federated_dl.py \
    ...
    --save-dir "/mnt/Data/Oni/artifacts_fullft_vramcheck" --seed 42
```

Run `watch -n 1 nvidia-smi` in a second terminal while this executes to watch
peak memory on whichever GPU index the run lands on (unset `CUDA_VISIBLE_DEVICES`
uses GPU 0 by default).

This is a fit check, **not** a metrics run — 2 rounds says nothing about quality.

**If it OOMs, work down this ladder — best comparability first:**
...
`torch.backends.cuda.matmul.allow_tf32 = True` (Ampere TF32; the 3090 is Ampere,
so this applies here; ~1.5–2× on matmuls, no memory change, negligible accuracy
cost), or bf16 `autocast` (roughly halves activation memory and 2–3× faster;
params stay fp32 so the FedProx proximal term is unaffected).
...
~6 GB for the root. Budget the disk (check with `df -h /mnt/Data`).
...
These two roots are independent of each other and of every other root — a good
pair to run in parallel on the two GPUs:
...
python compare_conditions.py \
    ...
    --metric mae --out-dir "/mnt/Data/Oni/comparisons/mu0.001_vs_mu0.01_temporal/mae"
python compare_conditions.py \
    --root-a "/mnt/Data/Oni/experiments_temporal_shared" --label-a "mu=0.01" \
    --root-b "/mnt/Data/Oni/experiments_temporal_mu0.001" --label-b "mu=0.001" \
    --metric cohen_kappa --out-dir "/mnt/Data/Oni/comparisons/mu0.001_vs_mu0.01_temporal/cohen_kappa"
python compare_conditions.py \
    --root-a "/mnt/Data/Oni/experiments_temporal_shared" --label-a "mu=0.01" \
    --root-b "/mnt/Data/Oni/experiments_temporal_mu0.1" --label-b "mu=0.1" \
    --metric mae --out-dir "/mnt/Data/Oni/comparisons/mu0.1_vs_mu0.01_temporal/mae"
python compare_conditions.py \
    --root-a "/mnt/Data/Oni/experiments_temporal_shared" --label-a "mu=0.01" \
    --root-b "/mnt/Data/Oni/experiments_temporal_mu0.1" --label-b "mu=0.1" \
    --metric cohen_kappa --out-dir "/mnt/Data/Oni/comparisons/mu0.1_vs_mu0.01_temporal/cohen_kappa"
```
...
python train_federated_dl.py \
    ...
    --save-dir "/mnt/Data/Oni/artifacts_lopo_personalized_s42" --seed 42
...
python train_federated_dl.py \
    ...
    --prox-mu 1e-2 --split-mode temporal --head-type corn \
    --holdout-project Hyperledger_Sawtooth --checkpoint-every 5 --resume \
    --save-dir "/mnt/Data/Oni/artifacts_lopo_shared_s42" --seed 42
```

These two are independent artifacts and can also run in parallel on the two
GPUs (`CUDA_VISIBLE_DEVICES=0` / `=1`, same pattern as the μ sweep above).
...
```bash
python run_lopo.py \
    --artifact-dir "/mnt/Data/Oni/artifacts_lopo_personalized_s42/federated" \
    --data-dir "/mnt/code/Oni/Shohel Ahmed CSE/federated-story-point-estimation/data_to_train_on_clean" \
    --holdout-project Hyperledger_Sawtooth --head-init generic \
    --budgets 0,10,25,50,100,200,400,758 --out-dir "/mnt/Data/Oni/lopo_results" --seed 42

python run_lopo.py \
    --artifact-dir "/mnt/Data/Oni/artifacts_lopo_shared_s42/federated" \
    --data-dir "/mnt/code/Oni/Shohel Ahmed CSE/federated-story-point-estimation/data_to_train_on_clean" \
...
```bash
python compare_lopo.py \
    --results-dir "/mnt/Data/Oni/lopo_results" \
    --out "/mnt/Data/Oni/lopo_results/lopo_comparison.csv"
```
...
```bash
python compute_statistics.py --experiments-root "/mnt/Data/Oni/experiments_random_shared" --metric mae --out-dir "/mnt/Data/Oni/experiments_random_shared/statistics_mae"
python compute_statistics.py --experiments-root "/mnt/Data/Oni/experiments_random_shared" --metric cohen_kappa --out-dir "/mnt/Data/Oni/experiments_random_shared/statistics_cohen_kappa"
python compute_statistics.py --experiments-root "/mnt/Data/Oni/experiments_random_shared" --metric macro_f1 --out-dir "/mnt/Data/Oni/experiments_random_shared/statistics_macro_f1"
...
python compare_conditions.py \
    ...
    --metric mae --out-dir "/mnt/Data/Oni/comparisons/rq2_shared_vs_phead_random/mae"
python compare_conditions.py \
    --root-a "/mnt/Data/Oni/experiments_random_shared" --label-a "Shared-head" \
    --root-b "/mnt/Data/Oni/experiments_random_personalized" --label-b "P-head" \
    --metric cohen_kappa --out-dir "/mnt/Data/Oni/comparisons/rq2_shared_vs_phead_random/cohen_kappa"
python compare_conditions.py \
    --root-a "/mnt/Data/Oni/experiments_random_shared" --label-a "Shared-head" \
    --root-b "/mnt/Data/Oni/experiments_random_personalized" --label-b "P-head" \
    --metric macro_f1 --out-dir "/mnt/Data/Oni/comparisons/rq2_shared_vs_phead_random/macro_f1"
```
...
7. LOPO case study (independent; can run in a parallel session — e.g. on the
   second GPU while step 3–5's root is still training on the first).
8. Random-split robustness roots.
9. μ sweep — **cut this first if compute runs short.**

With two GPUs, independent roots (steps 3 vs 7, or the two μ-sweep roots in
step 9) can overlap using `CUDA_VISIBLE_DEVICES=0`/`=1` as shown above — this
roughly halves wall-clock time for those pairs, though total GPU-hours below is
unchanged.
